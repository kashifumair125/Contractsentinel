...
async def agent_redline_suggester(clauses: list, risk_scores: list) -> dict:
    high_risk_ids = {r["clause_id"] for r in risk_scores if r.get("risk_level") in ["HIGH", "MEDIUM"]}
    risky_clauses = [c for c in clauses if c["id"] in high_risk_ids]
...
{{
  "redlines": [
    ...
    # Gemini Flash — fast single call, well within free tier for one agent
    try:
